...
// GET /api/accounting/settlement/staff-salaries?staffId=xxx
...
// POST /api/accounting/settlement/staff-salaries - 급여 추가/수정 (upsert)
export async function POST(request: NextRequest) {
  try {
    ...
    const { staff_id, month, amount, note } = body;

    if (!staff_id || !month) {
      return NextResponse.json({ error: '스태프 ID와 월은 필수입니다.' }, { status: 400 });
    }
...
// DELETE /api/accounting/settlement/staff-salaries?id=xxx
export async function DELETE(request: NextRequest) {
  try {
    ...
    const { error } = await supabase.from('rs_staff_salaries').delete().eq('id', id);
    if (error) {
