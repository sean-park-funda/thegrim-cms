...
export async function GET(request: NextRequest) {
  try {
    ...
    if (error) {
      console.error('지급 조정 조회 오류:', error);
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!partner_id || !month || !label || amount == null) {
    ...
    if (error) {
      console.error('지급 조정 생성 오류:', error);
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const { error } = await supabase.from('rs_payment_adjustments').delete().eq('id', id);
    if (error) {
      console.error('지급 조정 삭제 오류:', error);
