...
export async function GET(
  ...
  console.log('[movie][characters][GET] 캐릭터 목록 조회:', projectId);
  ...
  if (error) {
    console.error('[movie][characters][GET] 조회 실패:', error);
...
export async function POST(
  ...
  const { projectId } = await params;
  console.log('[movie][characters][POST] 캐릭터 추가:', projectId);
  ...
  if (body?.imageBase64) {
    ...
    if (uploadError) {
      console.error('[movie][characters][POST] 이미지 업로드 실패:', uploadError);
    ...
    if (urlData?.publicUrl) {
      ...
      console.log(`[movie][characters][POST] 이미지 URL 생성 성공 (${name}):`, imagePath);
    } else {
      // getPublicUrl이 실패한 경우 직접 URL 생성
      const supabaseUrl = process.env.NEXT_PUBLIC_SUPABASE_URL;
      imagePath = `${supabaseUrl}/storage/v1/object/public/movie-videos/${fileName}`;
      console.log(`[movie][characters][POST] 이미지 URL 폴백 사용 (${name}):`, imagePath);
    }
  }
  ...
  if (error) {
    console.error('[movie][characters][POST] 생성 실패:', error);
...
// PUT: /api/movie/[projectId]/characters - 캐릭터 일괄 업데이트
export async function PUT(
  request: NextRequest,
  { params }: { params: Promise<{ projectId: string }> }
) {
  const { projectId } = await params;
  console.log('[movie][characters][PUT] 캐릭터 일괄 업데이트:', projectId);
  ...
  for (const char of body.characters) {
    ...
    if (char.imageBase64) {
      ...
      if (uploadError) {
        console.error(`[movie][characters][PUT] 이미지 업로드 실패 (${char.name}):`, uploadError);
      } else {
        // 업로드 성공 시 공개 URL 생성
        const { data: urlData } = supabase.storage
          .from('movie-videos')
        ...
        if (urlData?.publicUrl) {
          imagePath = urlData.publicUrl;
          console.log(`[movie][characters][PUT] 이미지 URL 생성 성공 (${char.name}):`, imagePath);
        } else {
          // getPublicUrl이 실패한 경우 직접 URL 생성
          const supabaseUrl = process.env.NEXT_PUBLIC_SUPABASE_URL;
          imagePath = `${supabaseUrl}/storage/v1/object/public/movie-videos/${fileName}`;
          console.log(`[movie][characters][PUT] 이미지 URL 폴백 사용 (${char.name}):`, imagePath);
        }
      }
      ...
      console.log(`[movie][characters][PUT] 외부 이미지 경로 사용 (${char.name}):`, imagePath);
    } else if (char.id) {
      ...
      if (existing) {
        ...
        console.log(`[movie][characters][PUT] 기존 이미지 유지 (${char.name}):`, imagePath);
      }
    }
  ...
  if (charactersToInsert.length > 0) {
    ...
    if (error) {
      console.error('[movie][characters][PUT] 생성 실패:', error);
