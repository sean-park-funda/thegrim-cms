...
// GET: /api/movie/[projectId]/preview-image-prompt - 이미지 생성 프롬프트 미리보기
export async function GET(
  ...
  console.log('[movie][preview-image-prompt][GET] 프롬프트 미리보기:', projectId, '스타일:', style, '그리드:', gridSize);
  ...
  if (projectError || !project) {
    console.error('[movie][preview-image-prompt][GET] 프로젝트 조회 실패:', projectError);
...
`).join('')}`;
    ...
    if (panelDescriptions.length !== gridConfig.panelCount) {
      console.warn(`[movie][preview-image-prompt] 패널 설명 개수 불일치. 기대: ${gridConfig.panelCount}, 실제: ${panelDescriptions.length}`);
    }
  }
