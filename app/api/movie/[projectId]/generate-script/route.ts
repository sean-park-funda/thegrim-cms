...
// POST: /api/movie/[projectId]/generate-script - 영상용 스크립트 생성
export async function POST(
  request: NextRequest,
  { params }: { params: Promise<{ projectId: string }> }
) {
  const { projectId } = await params;
  console.log('[movie][generate-script][POST] 영상 스크립트 생성:', projectId);
  ...
  console.log('[movie][generate-script] 선택된 모델:', selectedModel);
  console.log('[movie][generate-script] 그리드 크기:', gridSize, '패널:', gridConfig.panelCount);
  console.log('[movie][generate-script] 영상 모드:', videoMode, '씬:', sceneCount);
  ...
  if (projectError || !project) {
    console.error('[movie][generate-script][POST] 프로젝트 조회 실패:', projectError);
    return NextResponse.json({ error: '프로젝트를 찾을 수 없습니다.' }, { status: 404 });
  }

  const scenes = project.movie_scenes as Array<{
    id: string;
    scene_index: number;
  ...
  for (const char of characters) {
    ...
    if (!imageUrl && char.storage_path && supabaseUrl) {
      imageUrl = `${supabaseUrl}/storage/v1/object/public/movie-videos/${char.storage_path}`;
      console.log(`[movie][generate-script] storage_path로 URL 생성: ${char.name} -> ${imageUrl}`);
    }
    ...
    if (imageUrl) {
      try {
        ...
        if (response.ok) {
          ...
          console.log(`[movie][generate-script] 캐릭터 이미지 로드: ${char.name}`);
        }
      } catch (err) {
        console.error(`[movie][generate-script] 캐릭터 이미지 로드 실패: ${char.name}`, err);
      }
    }
  }

  const hasCharacterImages = characterImages.length > 0;
  console.log(`[movie][generate-script] 캐릭터 이미지 ${characterImages.length}개 로드됨, 이미지 사용: ${hasCharacterImages}`);
...
- 예시: "A tall muscular man with a thick mustache, wearing a white sleeveless tank top..."`;
  ...
  if (videoMode === 'per-cut') {
    ...
    systemPrompt = `당신은 영화 영상 제작 전문가입니다.
주어진 대본을 바탕으로:
...
6. veoPrompt는 영어로 작성하되, 대사 부분만 원본 언어(한국어) 유지`;
  ...
  try {
    ...
    console.log('[movie][generate-script] 전체 프롬프트:');
    console.log('='.repeat(80));
    ...
    if (characterImages.length > 0) {
      console.log(`[movie][generate-script] ${characterImages.length}개 캐릭터 이미지 첨부`);
    ...
    console.log('='.repeat(80));
    console.log('[movie][generate-script] Gemini 응답 객체:');
    console.log('='.repeat(80));
    ...
    console.log('[movie][generate-script] responseText 길이:', responseText.length);
    if (responseText.length > 0) {
      console.log('[movie][generate-script] responseText 내용 (처음 2000자):');
      console.log(responseText.slice(0, 2000));
    } else {
      console.log('[movie][generate-script] 응답이 비었습니다! 전체 response:', JSON.stringify(response, null, 2).slice(0, 3000));
    }
    ...
    try {
      ...
      console.log('[movie][generate-script] 직접 JSON 파싱 실패, 코드블록 추출 시도...');
      ...
      if (jsonMatch) {
        try {
          ...
          console.error('[movie][generate-script] 코드블록 JSON 파싱 실패:', innerError);
          console.log('[movie][generate-script] 코드블록 내용:', jsonMatch[1].slice(0, 500));
        ...
        if (jsonObjectMatch) {
          try {
            ...
            console.error('[movie][generate-script] JSON 객체 파싱 실패:', objError);
            throw new Error('응답을 JSON으로 파싱할 수 없습니다.');
          }
        } else {
          console.error('[movie][generate-script] JSON을 찾을 수 없음. 전체 응답:', responseText);
          throw new Error('응답을 JSON으로 파싱할 수 없습니다.');
        }
      }
    }
    ...
    console.log('[movie][generate-script] 생성 완료:', {
    ...
    console.error('[movie][generate-script] 스크립트 생성 실패:', err);
    return NextResponse.json(
