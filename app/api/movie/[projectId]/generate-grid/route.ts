...
// POST: /api/movie/[projectId]/generate-grid - 그리드 이미지 생성
export async function POST(
  ...
  console.log('[movie][generate-grid][POST] 그리드 이미지 생성:', projectId, '스타일:', style, '그리드:', gridSize, '영상모드:', videoMode, '모델:', apiProvider);
  ...
  if (projectError || !project) {
    console.error('[movie][generate-grid][POST] 프로젝트 조회 실패:', projectError);
  ...
  for (const char of characters) {
    ...
    if (char.image_path) {
      try {
        ...
        console.warn(`[movie][generate-grid] 캐릭터 이미지 로드 실패: ${char.name}`, err);
      }
    }
  }
...
`).join('')}`;
    ...
    if (panelDescriptions.length === gridConfig.panelCount) {
      console.log('[movie][generate-grid] 패널 설명 사용:', panelDescriptions.length);
    } else {
      console.warn(`[movie][generate-grid] 패널 설명 개수 불일치. 기대: ${gridConfig.panelCount}, 실제: ${panelDescriptions.length}. 패널 설명을 사용하되 개수 불일치 주의.`);
    }
  } else {
    console.log('[movie][generate-grid] 패널 설명 없음, 대본 기반 생성');
  }
...
Create the complete ${gridSize} grid image with EXACTLY ${panelCount} equal-sized panels now.`;
  ...
  console.log('[movie][generate-grid] 전체 프롬프트:');
  console.log('='.repeat(80));
  console.log(promptText);
  console.log('='.repeat(80));
  console.log('[movie][generate-grid] 캐릭터 이미지 수:', characterParts.filter(p => 'inlineData' in p).length);
  ...
  try {
    ...
    if (uploadError) {
      console.error('[movie][generate-grid] 그리드 이미지 업로드 실패:', uploadError);
    ...
    if (sceneError) {
      console.error('[movie][generate-grid] 씬 저장 실패:', sceneError);
    }
    ...
    console.error('[movie][generate-grid] 이미지 생성 실패:', err);
    return NextResponse.json(
