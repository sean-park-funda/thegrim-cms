...
// POST: /api/shorts/[projectId]/generate-panel - 개별 패널 이미지 생성
export async function POST(
  ...
  console.log('[shorts][generate-panel][POST] 개별 패널 이미지 생성:', projectId, '패널:', panelIndex, '스타일:', style, '모델:', apiProvider);
  ...
  if (projectError || !project) {
    console.error('[shorts][generate-panel][POST] 프로젝트 조회 실패:', projectError);
  ...
  for (const char of characters) {
    ...
    if (char.image_path) {
      try {
        ...
        console.warn(`[shorts][generate-panel] 캐릭터 이미지 로드 실패: ${char.name}`, err);
      }
    }
  }
...
Create the single panel image now.`;
  ...
  console.log('[shorts][generate-panel] 패널 프롬프트:', promptText.slice(0, 500));
  ...
  try {
    ...
    if (uploadError) {
      console.error('[shorts][generate-panel] 이미지 업로드 실패:', uploadError);
    ...
    // shorts_scenes 테이블에서 해당 패널에 대응하는 씬 업데이트
    ...
    const existingScenes = project.shorts_scenes as Array<{
      id: string;
      scene_index: number;
    ...
    console.log('[shorts][generate-panel] 패널 생성 완료:', panelIndex, panelImagePath);
    ...
    console.error('[shorts][generate-panel] 이미지 생성 실패:', err);
    return NextResponse.json(
