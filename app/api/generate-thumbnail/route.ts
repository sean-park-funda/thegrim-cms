...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!fileId) {
      console.error('[썸네일 생성] fileId가 없음');
    ...
    // 파일 정보 조회
    console.log('[썸네일 생성] 파일 정보 조회:', fileId);
    ...
    if (fetchError || !file) {
      console.error('[썸네일 생성] 파일 조회 실패:', fetchError);
      return NextResponse.json(
        { error: '파일을 찾을 수 없습니다.' },
        { status: 404 }
      );
    ...
    if (file.file_type !== 'image') {
      console.error('[썸네일 생성] 이미지 파일이 아님:', file.file_type);
      return NextResponse.json(
    ...
    if (file.thumbnail_path) {
      console.log('[썸네일 생성] 이미 썸네일이 존재함:', file.thumbnail_path);
    ...
    // 원본 이미지 다운로드
    console.log('[썸네일 생성] 원본 이미지 다운로드 시작:', file.file_path);
    const imageResponse = await fetch(file.file_path);

    if (!imageResponse.ok) {
      console.error('[썸네일 생성] 이미지 다운로드 실패:', {
    ...
    console.log('[썸네일 생성] 이미지 다운로드 완료:', {
      size: imageSize,
      filePath: file.file_path
    });

    // 썸네일 생성 (최대 너비 400px, 품질 80%, 비율 유지)
    console.log('[썸네일 생성] 썸네일 생성 시작...');
    ...
    // 썸네일 Storage 경로 생성
    // 원본: {cutId}/{processId}/{fileName}
    // 썸네일: {cutId}/{processId}/thumbnails/{fileName}
    const originalPathParts = file.storage_path.split('/');
    const fileName = originalPathParts[originalPathParts.length - 1];
    const thumbnailPath = `${originalPathParts[0]}/${originalPathParts[1]}/thumbnails/${fileName}`;

    // Storage에 썸네일 업로드
    console.log('[썸네일 생성] Storage 업로드 시작:', thumbnailPath);
    ...
    if (uploadError) {
      console.error('[썸네일 생성] Storage 업로드 실패:', uploadError);
      return NextResponse.json(
    ...
    console.log('[썸네일 생성] Storage 업로드 완료');

    // DB에 thumbnail_path 업데이트
    console.log('[썸네일 생성] DB 업데이트 시작...');
    ...
    if (updateError) {
      console.error('[썸네일 생성] DB 업데이트 실패:', updateError);
