...
| 기능 | 관리자 (admin) | 매니저 (manager) | 회계 (accountant) | 스태프 (staff) | 조회자 (viewer) |
|------|---------------|-----------------|------------------|---------------|----------------|
| **데이터 조회** |
| 웹툰/회차/컷 조회 | ✅ | ✅ | ✅ | ✅ | ✅ |
| 파일 조회 | ✅ | ✅ | ✅ | ✅ | ✅ |
...
- ✅ **데이터 조회**: 모든 웹툰/회차/컷/파일 조회
- ✅ **회계 데이터 관리**: 수입/지출 거래 내역 생성/수정/삭제
- ✅ **예산 관리**: 웹툰별 예산 설정 및 진행률 추적
- ✅ **보고서 생성**: 월별/프로젝트별 회계 보고서 조회 및 내보내기
- ✅ **카테고리 관리**: 수입/지출 카테고리 추가/수정
- ❌ **콘텐츠 관리 불가**: 웹툰/회차/컷 생성/수정/삭제 불가
- ❌ **파일 업로드 불가**: 작업 파일 업로드 불가
- ❌ **거래 승인 불가**: 거래 내역 승인은 관리자만 가능
- ❌ **사용자 관리 불가**
...
관리자는 관리자 페이지 (`/admin`)에서 모든 사용자의 역할을 변경할 수 있습니다.
- 사용자 목록에서 드롭다운으로 역할 선택
- 즉시 적용되며 사용자는 다음 로그인 시 새로운 권한으로 접근 가능
...
⚠️ **중요**: 
- ✅ `user_profiles`와 `invitations` 테이블에 RLS 정책이 적용되어 있습니다.
- ⚠️ 웹툰/회차/컷/파일/공정 테이블은 현재 개발용으로 모든 사용자에게 접근 허용되어 있습니다.
- 프로덕션 배포 전에는 반드시 **역할별 접근 권한을 정의한 RLS 정책**을 데이터베이스 레벨에서 설정해야 합니다.
