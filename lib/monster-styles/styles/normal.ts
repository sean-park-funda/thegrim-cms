...
4. **표현:** '단순한 결합'을 넘어 생물학적으로 불쾌한 변형(Body Horror)을 묘사할 것.
...
6. **구도:**
   ...
   - **전신 표시:** 괴수의 전체가 보이도록 전신 샷
   - **중앙 배치:** 괴수가 이미지 중앙에 위치

7. **🚫 절대 포함하지 말 것:**
   - **텍스트 금지:** 글자, 대사, 말풍선 금지

8. **이미지 비율:**
   - 9:16 (세로), 1:1 (정사각형), 16:9 (가로) 중 괴수 형태에 맞게 선택

**중요:** 응답은 반드시 유효한 JSON 형식으로 작성해주세요:
\`\`\`json
{
  "imagePrompt": "실제 생성에 사용할 상세한 영어 프롬프트. 각 섹션에서 선택된 생물이 해당 부위의 기본 구조가 되도록. --no text, background, scenery 포함",
  "aspectRatio": "9:16 또는 1:1 또는 16:9 중 하나"
}
\`\`\`
